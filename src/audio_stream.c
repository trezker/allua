...
ALLUA_audio_stream allua_check_audio_stream (lua_State *L, int index)//, int *gc_allowed)
{
  ...
  pi = (struct ALLUA_audio_stream_s*)luaL_checkudata(L, index, AUDIO_STREAM_STRING);
  if (pi == NULL)
...
{
  struct ALLUA_audio_stream_s *pi = (struct ALLUA_audio_stream_s *)lua_newuserdata(L, sizeof(struct ALLUA_audio_stream_s));
  pi->audio_stream = im;
...
{
  struct ALLUA_audio_stream_s *pi = (struct ALLUA_audio_stream_s*)lua_touserdata(L, 1);
  if(pi->gc_allowed)
  {
